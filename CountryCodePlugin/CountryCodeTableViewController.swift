...
public class CountryCodeTableViewController: UITableViewController {
    ...
    private func loadData() {
        NSLog(Bundle(for: type(of: self)).path(forResource: "CountryCodeList\(language)", ofType: "plist") ?? "nil")
        NSLog(Bundle(for: type(of: self)).path(forResource: "CountryCodeList\(language)", ofType: "plist", inDirectory: "Resources") ?? "nil")
        NSLog(Bundle(for: type(of: self)).url(forResource: "CountryCodeList\(language)", withExtension: "plist")?.absoluteString ?? "nil")
        NSLog(Bundle(for: type(of: self)).url(forResource: "CountryCodeList\(language)", withExtension: "plist", subdirectory: "Resources")?.absoluteString ?? "nil")
