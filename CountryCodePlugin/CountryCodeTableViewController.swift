...
public class CountryCodeTableViewController: UITableViewController {
    ...
    private func loadData() {
        if let URL = Bundle(for: type(of: self)).path(forResource: "Resources/CountryCodeList\(language)", ofType: "plist"), let list = NSArray(contentsOfFile: URL) as? [NSDictionary] {
            for item in list {
